...
{
    /// <summary>Hex Residue Node -- spell damage ward objective. Destroy to stabilize. Mod-original.</summary>
    public class HexResidueNode : ModNPC
    {
        ...
        {
            NPC.width = 20; NPC.height = 20; NPC.damage = 15; NPC.defense = 8; NPC.lifeMax = 250;
        ...
        public override void AI()
        {
            ...
            Lighting.AddLight(NPC.Center, 0.3f, 0.1f, 0.3f);
            if (Main.rand.NextBool(3)) { Dust d = Dust.NewDustDirect(NPC.position, NPC.width, NPC.height, DustID.PurpleTorch, 0f, -0.5f, 100, default, 0.8f); d.noGravity = true; }
            if (Main.GameUpdateCount % 120 == 0)
                foreach (Player p in Main.player)
                    if (p.active && !p.dead && Vector2.Distance(p.Center, NPC.Center) < 150f)
                        p.AddBuff(BuffID.ManaSickness, 90);
        }
